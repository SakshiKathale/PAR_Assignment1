...
def setupBash():
    ...
    if setupRobot:
        ...
        query = True #query_yes_no("Configure root ~/.bashrc to automatically source ROSbot workspace?")
...
if __name__ == "__main__":
    ...
    # Setup bash script
    query = True #query_yes_no("Configure Bash? (Includes ROS envrionment parameters")
    if query:
        setupBash()
        #exit()
    print()
    ...
    # Install software
    query = True #query_yes_no("Install General Software?")
    if query:
        print_subitem("Installing software")
        installSoftware(configSoftware)
    print()

    query = True #query_yes_no("Install ROS Specific Additional Packages?")
    if query:
        rosversion='none'
        if setupRobot:
            rosversion=configRobots[setupRobotName]["rosversion"]
    ...
    # Setup Git
    if setupComp:
        query = False #query_yes_no("(Computer only) Setup Git?")
        if query:
            setupGit()
        print()

    # Setup SSH Config
    query = False #query_yes_no("Setup SSH Config?")
    if query:
        setupSSHConfig(configRobots, configComputers)
    print()

    # Setup /etc/hosts Aliases
    query = False #query_yes_no("Setup /etc/hosts alias?")
    if query:
    ...
    # Setup SSH Keys
    if setupComp:
        query = False #query_yes_no("(Computer only) Setup SSH Keys?")
        if query:
            setupSSHKeys()
        print()
    ...
    if setupRobot:
        query = False #query_yes_no("(Robot only) Replace authorised SSH keys?")
        if query:
            replaceAuthKeys()
        print()

    # Configure Husarion Git Repositories
    query = True #query_yes_no("Configure & Build Repositories?")
    if query:
    ...
    if setupRobot:
        query = False #query_yes_no("Configure Greengrass for AWS remote connection")
        if query:
